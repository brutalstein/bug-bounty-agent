...
def derive_authorized_surface_targets(scope: ScopeManager, seed_target: str) -> list[str]:
...
def command_session_compare_run(args: argparse.Namespace) -> int:
    ...
    if summary.items:
        changed_items = [item for item in summary.items if item.get("status_changed") or item.get("accessibility_changed")]
        if changed_items:
